from PlotFunctions import plot_rectangle
import tkinter as tk
class SideMenu(tk.Frame):
    def __init__(self, root):
        super().__init__(root, width = 400)
        self.configure(bg = '#314457')
        self.root = root
        self.shape = None
        self.controls=[]
        ...
        # self.c1 = tk.Checkbutton(self, text = 'koordináta rendszer', variable = coordinate_on, onvalue = True, offvalue = False, command = plot_rectangleplot_rectangle(self,a,b, coordinate_on, True))
        #self.e1.grid(row=2)
        #self.e2.grid(row=4)
...
# DEFINE FUNCTIONS -----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def change_to_circle(self):
        self.controls[0]["nev"].config(text="d")
        self.controls[0]["nev"].grid(row=2,column=0)
        self.controls[0]["entry"].grid(row=2,column=1)
        self.controls[0]["mertekegyseg"].grid(row=2,column=2)
        self.calc.grid(row=4,column=1,pady=5)
